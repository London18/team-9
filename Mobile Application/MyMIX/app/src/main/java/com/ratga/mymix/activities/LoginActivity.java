...
import com.ratga.mymix.daos.Sessions;
...
public class LoginActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        anonymousSignInButton.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                final User anonymous = new User("guest", "guest", "", 0, "");
                Users.addUser(anonymous, new Callback<User>() {
                    @Override
                    public void onResponse(Call<User> call, final Response<User> response) {
                        final Session session = new Session(response.body().getUserId(),response.body());
                        Sessions.addSession(session, new Callback<Session>() {
                            @Override
                            public void onResponse(Call<Session> call, Response<Session> response2) {
                                Intent intent = new Intent(context, ChatActivity.class);
                                intent.putExtra("user", response.body());
                                intent.putExtra("session", response2.body());
